...
========
This data is freely available, but please cite the source. The citation can be fetched programmatically this way.
...
--------
Read_delim parses some columns incorrectly, probably since the file starts with lots of null values. We here specify the data types manually.
...
We then split the event data into section events (with parent ID), and transect events (without parent ID's). If we would have more hierarchical levels, this would have been more convoluted.
